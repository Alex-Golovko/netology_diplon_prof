...
class VKinder:
    def __init__(self, token = None, c_token = None):
        ...
        if self.token and self.c_token:
            ...
            self.c_vk = vk_api.VkApi(token=c_token)
            self.c_lp = VkLongPoll(vk_api.VkApi(token=self.c_token))

            try:
                self.user_get()
            except vk_api.exception.ApiError as error_msg:
                print(error_msg)
                sys.exit()

    def user_get(self, id_ = None):
        try:
    ...
    def get_prof_photos(self, id_):
        try:
            profile = self.vk.photos.get(owener_id=id_, album_id = 'profile', extended = 1)
            time.sleep(0.3)
        except vk_api.exceptions.ApiError as error_msg:
            print(error_msg)
            return
    ...
    def write_msg(self, user_id, message):
        self.c_vk.method('message.send', {'user_id': user_id, 'message': message, 'random_id': randrange(10 ** 7), })
